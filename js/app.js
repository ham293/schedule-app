...
(function () {
  ...
  async function scheduleNativeReminders() {
    ...
    try {
      ...
      await LN.createChannel({ id: 'alarm_max', name: '日程提醒', importance: 5, sound: 'schedule_alarm', vibration: true, visibility: 0, lights: true });
    } catch (e) { /* 忽略 */ }
    const now = Date.now();
    const list = [];
    for (const it of items) {
      ...
      if (when > now) {
        list.push({
          ...
          schedule: { at: new Date(when).toISOString(), allowWhileIdle: true },
    ...
    $('#notifyBtn').addEventListener('click', async () => {
      ...
      if (LN) {
        try {
          ...
          await LN.schedule({ notifications: [{ id: 123456, title: '⏰ 测试提醒', body: '收到说明后台提醒正常', schedule: { at: new Date(Date.now() + 2000).toISOString(), allowWhileIdle: true }, sound: 'schedule_alarm', channelId: 'alarm_max', smallIcon: 'ic_stat_icon' }] });
